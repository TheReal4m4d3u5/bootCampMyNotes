...
function updateLocalStorage() {
  ...
  localStorage.setItem('tempStorageObjectLabel', JSON.stringify(tempStorageObject));
}

// ? Function to load from local storage. This function will be called on page load.
function loadFromLocalStorage() {
  const storedData = JSON.parse(localStorage.getItem('tempStorageObjectLabel'));
  if (storedData) {
...
function attachMouseListeners() {
  // TODO: Attach the mouse move event listener to the document and the click listener to the mood board div so that the element can be dragged anywhere on the screen,
