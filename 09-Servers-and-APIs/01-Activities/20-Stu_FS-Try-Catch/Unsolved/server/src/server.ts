...
app.post('/api/contacts', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const readContacts: string = await fs.readFile('src/db/contacts.json', 'utf-8');

    const parsedContacts: Contact[] = JSON.parse(readContacts);

 
    parsedContacts.push(contact);
 

    await fs.writeFile('src/db/contacts.json', JSON.stringify(parsedContacts));
    return res.status(201).json(parsedContacts);
