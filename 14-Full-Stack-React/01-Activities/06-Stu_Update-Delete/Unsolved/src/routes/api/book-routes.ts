...
// try {
//   await Book.update(
//    { title, author, isbn, pages, edition, is_paperback }, // Fields to update
//    {
//      where: { book_id: id }, // Condition to find the specific book by ID
//    }
//  );
 // if (updated) {
 //   const updatedBook = await Book.findByPk(id); // Fetch the updated book
 //   res.json(updatedBook); // Return the updated book
 // } else {
 //   res.status(404).json({ message: 'Book not found' }); // If no book was found with the ID
 // }
 // } catch (error: any) {
...
export const updateBook = async (req: Request, res: Response) => {
 ...
 // const { id } = req.params; // Get the book ID from the URL
  ...
  try{
    const udpatedBook = await Book.update(
      { ...req.body,
      // saying the same thing  title: req.body.title, ... const { title, author, isbn, pages, edition, is_paperback } = req.body; // Get updated data from the request body
       }, // Fields to update
      {
        where: { book_id: req.params.id }, // Condition to find the specific book by ID
      }
    )
  ...
  //     where: { book_id: id }, // Find the book by its ID
  //   });

  //   if (deleted) {
  //     res.json({ message: 'Book deleted successfully' }); // Confirm deletion
  //   } else {
  //     res.status(404).json({ message: 'Book not found' }); // If no book matches the ID
  //   }
  // } catch (error: any) {
  //   res.status(500).json({ message: error.message }); // Handle any errors
  // }
...
// DELETE /Books/:id
export const deleteBook = async (req: Request, _res: Response) => {
 ...
 // const { id } = req.params; // Get the book ID from the URL
  ...
  try{
    ...
    const deleteBook = await Book.destroy({
      where: { book_id: req.params.id }, // Find the book by its ID
    }
