...
def show_list():
    ...
    if not shopping_list:
        ...
        for item_name, item in shopping_list.items():
            if "price" in item:
                # Format the price as a string with a dollar sign, comma, and
                # two decimal places
                price = f'${item["price"]:,.2f}'
            else:
                price = "unknown"

            # Add the item name, quantity, and formatted price string to the
            # formatted_items list
            formatted_items.append(f"{item_name}: {item['quantity']} @ "
                                f"{price} each")

            # Find the length of the longest item in the shopping list
            if len(formatted_items[-1]) > max_print_length:
                max_print_length = len(formatted_items[-1])

    # Create a border string to separate the title and items using the length
    # of the longest item
    ...
    for item in formatted_items:
        if len(item) < max_print_length:
            # Left justify the item in the border
            print(f"| {item.ljust(max_print_length)} |")
        else:
            # Print the item in the border
            print(f"| {item} |")
    print(border_string)
